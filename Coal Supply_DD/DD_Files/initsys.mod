...
* Numbered sets
  ...
  SET J            'Supply/demand steps 1*COM_STEP and SHAPE/MULTI' / 1*300 /
  ALIAS(J,JJ);
...
* Fixed sets for user-constraints
  SET SIDE     'LHS and RHS of an equation'
    / LHS
      RHS /;
...
* NOT COMPLETE
  ...
  SET UC_GRPTYPE 'Type of components within UC_GRP'
    / ACT, FLO, IRE, CAP, NCAP, COMNET, COMPRD, COMCON /;
...
$ SET RUN_NAME      'TEST'
* include the $IF DECLARED/NOT REG code in *.RUN
$ SET STARTRUN      'SCRATCH'
